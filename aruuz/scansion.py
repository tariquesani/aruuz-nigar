...
def length_five_scan(substr: str) -> str:
    ...
    Returns:
        Scansion code based on character patterns
    """
    logger.debug(f"length_five_scan: Input substring = '{substr}'")
    code = ""
    # Remove ھ and ں for scansion purposes
    sub_string = substr.replace("\u06BE", "").replace("\u06BA", "")
    logger.debug(f"length_five_scan: After removing ھ and ں = '{sub_string}'")
    stripped = remove_araab(sub_string)
    logger.debug(f"length_five_scan: After removing araab (stripped) = '{stripped}' (length={len(stripped)})")
    ...
    if len(stripped) == 3:
        ...
        if stripped[0] == 'آ':
            ...
            logger.debug(f"length_five_scan: Split at position 2 (آ pattern): prefix='{sub_string[:2]}', remaining='{remaining}'")
                ...
                # Position 2 Alif
                    ...
                    if len(loc) > 0 and is_muarrab(loc[0]):
                        if len(loc) > 1 and is_muarrab(loc[1]):
                            split_pos = 3
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (Position 2 Alif, muarrab[0] and muarrab[1]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                        else:
                            split_pos = 4
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (Position 2 Alif, muarrab[0] only): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                    else:
                        if len(loc) > 1 and is_muarrab(loc[1]):
                            split_pos = 2
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (Position 2 Alif, muarrab[1] only): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                        else:
                            split_pos = 3
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (Position 2 Alif, no muarrab): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                # Position 4 Alif
                else:
                    code = "==-"
                ...
                if len(stripped) > 1 and (stripped[1] == 'و' or stripped[2] == 'و' or stripped[3] == 'و' or
                        ...
                        if len(loc) > 1 and loc[1] == ARABIC_DIACRITICS[2]:  # jazr
                            if len(loc) > 0 and is_muarrab(loc[0]):
                                if len(loc) > 1 and is_muarrab(loc[1]):
                                    split_pos = 5
                                    remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                    logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr, muarrab[0] and muarrab[1]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                    code = "=" + length_three_scan(remaining)
                                else:
                                    split_pos = 4
                                    remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                    logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr, muarrab[0] only): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                    code = "=" + length_three_scan(remaining)
                            else:
                                if len(loc) > 1 and is_muarrab(loc[1]):
                                    split_pos = 3
                                    remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                    logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr, muarrab[1] only): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                    code = "=" + length_three_scan(remaining)
                                else:
                                    split_pos = 4
                                    remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                    logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr, no muarrab): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                    code = "=" + length_three_scan(remaining)
                        elif len(loc) > 1 and (loc[1] == ARABIC_DIACRITICS[1] or  # zer
                                               loc[1] == ARABIC_DIACRITICS[8] or  # zabar
                                               loc[1] == ARABIC_DIACRITICS[9]):  # paish
                            ...
                            if len(loc) > 2 and (loc[2] == ARABIC_DIACRITICS[1] or  # zer
                                ...
                                if len(loc) > 3 and (loc[3] == ARABIC_DIACRITICS[1] or  # zer
                                    ...
                                    if len(loc) > 2 and is_muarrab(loc[2]):
                                        split_pos = 4
                                        remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                        logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr at 2, muarrab[2]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                        code = "=" + length_three_scan(remaining)
                                    else:
                                        split_pos = 3
                                        remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                        logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, jazr at 2, no muarrab[2]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                        code = "=" + length_three_scan(remaining)
                            else:
                                split_pos = 2
                                remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                                logger.debug(f"length_five_scan: Split at position {split_pos} (Position 1 و/ی, no jazr at 2): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                                code = "=" + length_three_scan(remaining)
                    ...
                    if len(loc) > 1 and (loc[1] == ARABIC_DIACRITICS[1] or  # zer
                        ...
                        if len(loc) > 0 and is_muarrab(loc[0]):
                            split_pos = 4
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (no و/ی, jazr at 1, muarrab[0]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                        else:
                            split_pos = 3
                            remaining = sub_string[split_pos:] if len(sub_string) > split_pos else ""
                            logger.debug(f"length_five_scan: Split at position {split_pos} (no و/ی, jazr at 1, no muarrab[0]): prefix='{sub_string[:split_pos]}', remaining='{remaining}'")
                            code = "=" + length_three_scan(remaining)
                    elif len(loc) > 2 and (loc[2] == ARABIC_DIACRITICS[1] or  # zer
                                          loc[2] == ARABIC_DIACRITICS[8] or  # zabar
                                          loc[2] == ARABIC_DIACRITICS[9]):  # paish
    ...
    logger.debug(f"length_five_scan: Final code for '{substr}' = '{code}'")
    return code
