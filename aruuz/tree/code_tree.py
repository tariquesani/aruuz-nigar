...
class CodeTree:
    ...
    def _match_char(self, p: str, c: str) -> bool:
        """
    ...
    def _align(
        self, pattern: str, code: str
    ) -> Tuple[int, List[Dict[str, Any]], List[Tuple[int, int]]]:
        """
        Align pattern (meter) to code (line) via DP + backtrack; same x/~ rules as
        _levenshtein_distance. Returns (distance, edit_ops, leverage).
        
        edit_ops: list of {"op": "match"|"insert"|"delete"|"substitute", ...} with
                  pattern_pos / code_pos (0-based). insert = add to code to match pattern;
                  delete = remove from code.
        leverage: list of (code_start, code_end) exclusive-end ranges that already match.
        """
        m, n = len(pattern), len(code)
        d: List[List[int]] = [[0] * (n + 1) for _ in range(m + 1)]
        bp: List[List[Optional[Tuple[str, str]]]] = [
